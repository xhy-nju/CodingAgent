...
- 凭据：本地开发优先使用 OS keyring；Docker、CI、服务器使用环境变量或挂载 `.env`。任何模式都不得提交、打印或写入真实 provider token。
...
本地开发时，用户可通过 CLI 或 WebUI 把 provider token 写入 OS keyring。Docker、CI 和阿里云 Ubuntu 部署时，通过环境变量或挂载 `.env` 注入。`.env` 是明文文件，只能放在服务器本地并通过 `.gitignore` 排除。
